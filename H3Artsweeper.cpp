...
/*  1.生成界面
    ...
    4.雷，已打开的非雷块，未打开的遮罩层，标记可以通过[-1 + 10 * k, 8 + 10 *
   k](k -> Z)来表示
...
int Mouse_event(int *inix, int *iniy) {
  ...
  } else if (event.message == WM_MBUTTONDOWN && map[row][col] > 0 &&
             map[row][col] < 9) {
...
void Botton_line() {
  ...
  drawtext(_T("Copyright H3Art_q from Jinan University International School ")
           _T("2021 CST Major"),
...
int main() {
  ...
  MessageBox(
      ...
      TEXT(
          "这是一个扫雷游戏，但雷是H3Art_"
          "q的heart，\n基本操作规则只有左键打开未打开的方块，\n右键在你认为有he"
          "art的位置进行标记，\n以及，通过鼠标中键在已打开的数字区域快速打开周"
          "围九宫格内的方块，\n但是请注意，中键使用的前提是该数字周围的九宫格里"
          "做了正确数量和正确位置的标记，\n否则，你将不可避免地坠入爱河！"),
      TEXT("这是H3Art_q自己写的第一个不是黑框窗口的程序！可恶的C语言！"),
      MB_OK);
  // 创建游戏窗口
...
Label:
  ...
  while (true) {
    ...
    if (Open_count()) {
      ...
      Put_image();
      int result = MessageBox(NULL, TEXT(time_counter(time_start)),
                              TEXT("可恶！"), MB_RETRYCANCEL);
      switch (result) {
      case IDRETRY:
        goto Label;
        break;
